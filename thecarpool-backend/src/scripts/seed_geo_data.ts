...
const connectionString = process.env.DATABASE_URL || 'postgresql://postgres:thecarpoolsecretpassword@localhost:5432/thecarpool';
...
async function seed() {
  console.log('🌱 Starting global geographic database seeding...');
  const client = await pool.connect();
  try {
    await client.query('BEGIN');

    // Create tables if not exists (fail-safe fallback)
    await client.query(`
    ...
    // Create indices
    await client.query('CREATE INDEX IF NOT EXISTS idx_postal_codes_location ON postal_codes USING GIST(location);');
    await client.query('CREATE INDEX IF NOT EXISTS idx_postal_codes_code ON postal_codes(postal_code);');
    ...
    console.log('✓ Existing geographic database table data cleared.');
    ...
    for (const country of countriesToSeed) {
      ...
      console.log(`  -> Seeded Country: ${country.name} (${country.iso})`);
      ...
      for (const state of country.states) {
        ...
        console.log(`     └─ Seeded State: ${state.name} with ${state.postalCodes.length} postal codes.`);
      }
    }

    await client.query('COMMIT');
    console.log('🎉 Geographic database seeding completed successfully!');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('❌ Seeding failed, database changes rolled back:', error);
  } finally {
    client.release();
    await pool.end();
  }
